...
public class TagKeyBuilder {
	 ...
	 */
	public static <T> TagKey<T> build(ResourceKey<? extends Registry<T>> registry_key, String reslocWithNamespace) {
		return build(registry_key, ResourceLocationBuilder.build(reslocWithNamespace));
	}
	...
	public static <T> TagKey<T> build(ResourceKey<? extends Registry<T>> registry_key, String namespace, String path) {
		return build(registry_key, ResourceLocationBuilder.build(namespace, path));
	}
